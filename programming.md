...
Создаем новый проект > `mkdir hexlet-git`

Переходим в созданную директорию > `cd hexlet-git`

Выполняем инициализацию > `git init` (Initialized empty Git repository in /private/tmp/hexlet-git/.git/)

Создаем файл README.md со строкой текста > `echo 'Hello, Hexlet!' > README.md`

Для каждого нового или измененного файла > `git add README.md` (находится в состоянии «подготовлен к коммиту» — другими словами, файл попал в индекс.)
...
Во время коммита мы берем все подготовленные изменения (они могут включать любое количество файлов) и отправляем их в репозиторий как единое целое > `git commit -m 'add README.md'` 
